...
const QualityMenuItem = videojs.extend(MenuItem, {
	constructor: function(player, options) {
        options = videojs.mergeOptions({selectable: true, selected: options.active || false}, options);
...
const QualitySubMenu = videojs.extend(SubMenu, {
	...
	createItems: function () {
		this.player().currentSources().forEach(source => {
			let item = new QualityMenuItem(this.player(), {active: source.src === this.currentSource, label: this.localize(source.label), src: source.src, callback: this.handleClick, source: source});
			console.log(item);
	...
	updateCurrentSource: function (event, source) {
		this.menuItem.minorLabel.innerHTML = this.localize(source.label);
